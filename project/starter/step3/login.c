...
// NEW FUNCTION: Updates the counter in the file
    ...
    while (fscanf(fptr, "%s %s %s %d", f_user, f_salt_hex, f_hash_hex, &f_attempts) != EOF) {
        if (strcmp(in_user, f_user) == 0) {
            ...
            if (strcmp(generated_hash, f_hash_hex) == 0) {
                success = 1;
                update_counter(f_user, 0); // Reset counter on success
            } else {
                update_counter(f_user, f_attempts + 1); // Increment counter on failure
            }
            break;
        }
    }
